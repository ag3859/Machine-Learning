...
--	print(r:size())
--	print("r:size() in mcluster"..r:size())
--	io.read()
--	local rsize=r:size()
	rsize=rsize+1-1
      for i = 1, rsize do		--use x:size()[1]
	 m = m + x[i]*r[i]/w
      end
	
      -- Find difference between old and new mean to check for convergence
	convergence_diff=c:eval(m)
--	print("xx"..convergence_diff)
      -- Set the mean
      c.m = m
		
--	print("convergence_diff"..convergence_diff)
--	io.read()

	if convergence_diff<0.1 then
		return 1
	else 
		return 0
	end

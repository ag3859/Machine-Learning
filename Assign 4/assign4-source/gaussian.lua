...
function gaussian(n)
   ...
   function g:learn(x,r1,datasize,p)
	r=torch.Tensor(datasize):fill(r1)
--	print(x)
--	print("this is x")
--	print(x:size())
--	print("size of x")
--	io.read()
      ...
      local w = torch.sum(r)
      -- Weighted mean
      for i = 1,datasize do 			--x:size()[1] do
	 m = m + x[i]*r[i]/w
      end
      -- Weighted covariance
      for i = 1,datasize do				--use x:size()[1]
	 c = c + torch.ger(x[i]-m,x[i]-m)*r[i]/w
      end

	convergence_diff=torch.norm(m-g.m)

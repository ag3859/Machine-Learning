...
---------------------------------------------------------------------------CODE FOR CONVERGENCE DETECTION-----------------
--[[		local stop=false
		
		local loss=0;
		
		loss = loss*(j-1)/j + model:l(dataset[j][1], dataset[j][2])/j

		oldloss[j]=loss

		if  (j>26) then
			for k=1,25 do
				if torch.abs(oldloss[j-k]-oldloss[j-k-1])<0.001 then
					stop=true
				else stop=false
				end
			end			
		...
		if(j>26) then
			if(stop==true) then
				print("Convergence Achieved for iteration number "..j)
				break
			end
		end
]]

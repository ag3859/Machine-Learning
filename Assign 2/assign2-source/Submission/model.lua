...
--   print("You have to define this function by yourself! This is Primal SVM");
...
--	print("This is W \n")
--	print(model.w)
	local trainer=trainerSGD(model,stepCons(0.008))

	function model:l(x,y)
		return math.max(0,1-y[1]*torch.dot(model.w,x)) + r:l(model.w)
	end


	function model:dw(x,y)
--		print(x)
--		print("that was x, now w size is \n")
--		print(y[1])
--		print(model.w)
		if(math.max(0,1-y[1]*torch.dot(model.w,x))==0) then
			return torch.zeros(model.w:size()) + r:dw(model.w)
		end
